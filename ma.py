...
lvl1 = str(open('end_screen.txt', encoding="utf-8").read().strip()).split('\n')
...
if __name__ == '__main__':
    ...
    while running:
        ...
        if z_down and not keys[pygame.K_z]:
            z_down = False
        if c_down and not keys[pygame.K_c]:
            c_down = False
        ...
        render_crazy_rect()
        pygame.draw.rect(screen, pygame.Color(255, 255, 255), (check_coords(int(x), int(y)), (20, 20)), 0)
        pygame.draw.rect(screen, pygame.Color(255, 0, 0), (check_coords(int(x), int(y)), (3, 3)), 0)
        pygame.draw.rect(screen, pygame.Color(255, 0, 0), ((int(x) + 17, int(y)), (3, 3)), 0)
        coords = check_coords(int(x), int(y))
        pygame.display.flip()
    pygame.quit()