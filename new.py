...
def check_place(x_, y_):
    ...
    try:
        ...
        pass
    try:
        if lvl[int(y__ + 1)][int(x__)].strip() == 'TRAP' or lvl[int(y__)][int(x__)].strip() == 'TRAP' or \
                lvl[int(y__)][int(x__ + 1)].strip() == 'TRAP' or lvl[int(y__)][int(x__ - 1)].strip() == 'TRAP':
            return 'on the trap'
